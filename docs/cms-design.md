...
ขั้นตอนติดตั้งเซิร์ฟเวอร์ทั้งหมดอยู่ใน **[docs/deploy-plesk.md](./deploy-plesk.md)**
...
โฮสต์จริงเป็น **Plesk แบบแชร์ของ HostAtom** ซึ่งให้มาแค่ MariaDB และไม่มีสิทธิ์ root
ให้ติดตั้ง PostgreSQL เอง จึงเหลือ SQLite เป็นตัวเลือกเดียวที่ทำได้ — และบังเอิญว่า
เหมาะกับงานนี้พอดี: ผู้ดูแลไม่กี่คน ปริมาณเขียนต่ำ และทั้งเว็บสำรองได้ด้วยการคัดลอกไฟล์เดียว

> **ข้อแลกเปลี่ยน** — SQLite จัดคิวการเขียนทีละคำสั่ง ถ้าวันหนึ่งมีผู้ดูแลแก้เนื้อหาพร้อมกัน
> หลายคนหรือทราฟฟิกสูงขึ้นมาก ค่อยย้ายไป PostgreSQL ซึ่งเปลี่ยนแค่ adapter กับ migration
>
> **ผลต่อขั้นตอน deploy** — โฮสต์ปิด SSH ไว้ จึงรันคำสั่งบนเซิร์ฟเวอร์ไม่ได้เลย
> แอปจึงรัน migration ที่ค้างอยู่เองตอนเริ่มทำงาน (มีตัวล็อกกันหลายโพรเซสชนกัน)
> และสคริปต์ deploy ส่งไฟล์ผ่าน FTP แทน ดูคู่มือ deploy หัวข้อ 0 และ 8
...
| **หน้าที่สร้างเอง** | สร้างหน้าใหม่ทั้งหน้าจากบล็อก 7 ชนิด | Collection `pages` |
| **ป้ายกำกับที่ใช้ซ้ำทั้งเว็บ** | "ผลิตโดย" "สเปกทางเทคนิค" "เวลาทำการ" ฯลฯ รวม 48 คำ | Global `ui-labels` |
| **ทางเปลี่ยนเส้นทาง** | พาลิงก์เก่าไปหน้าใหม่เมื่อเปลี่ยน slug | Collection `redirects` |
...
> รวมถึงชื่อสถานะสินค้า (`พร้อมส่ง`/`สั่งทำล่วงหน้า`/`สินค้าหมด`), ชื่อรูปแบบผลิตภัณฑ์
> และตัวเลือกการเรียงลำดับ เพราะค่าเหล่านี้ผูกกับตรรกะของตัวกรอง ถ้าพิมพ์ผิดตัวกรองจะพัง
> และข้อความในเครื่องมือของผู้ดูแลเอง (แถบดูตัวอย่าง ปุ่มแก้ไข) ซึ่งผู้เข้าชมไม่เห็นอยู่แล้ว
...
> รูปสถานะ `pending` ถูกกันเฉพาะทางแก้บนหน้าเว็บ (ไม่แสดงในคลังรูปของตัวเลือก และ API ปฏิเสธ)
> ในหลังบ้านยังเลือกได้ — ผู้ดูแลต้องตรวจเอง
...
### 4.10 `redirects` — ทางเปลี่ยนเส้นทาง

เมื่อเปลี่ยน slug หรือลบหน้าทิ้ง ลิงก์ที่เคยแชร์ไว้ในไลน์หรือเฟซบุ๊กจะกลายเป็น 404
และอันดับใน Google ที่สะสมมาก็หายไปด้วย รายการในคอลเลกชันนี้พาลิงก์เก่าไปหน้าใหม่แทน

| ฟิลด์ | หมายเหตุ |
| --- | --- |
| ลิงก์เดิม | ใส่เฉพาะส่วนหลังโดเมน เช่น `/shop/old-balm` · ตัด `/` ท้ายให้อัตโนมัติ · ห้ามซ้ำ |
| ลิงก์ปลายทาง | เส้นทางในเว็บ หรือลิงก์เต็มที่ขึ้นต้นด้วย `https://` |
| ย้ายถาวร | ติ๊ก = 308 (Google โอนอันดับให้หน้าใหม่) · ไม่ติ๊ก = 307 |
| เปิดใช้งาน | ติ๊กออกเพื่อหยุดชั่วคราวโดยไม่ต้องลบ |
| บันทึกช่วยจำ | เช่น "เปลี่ยนชื่อสินค้าเมื่อ ก.ย. 2569" |

**ทำงานตอนที่หาหน้าไม่เจอเท่านั้น** — ไม่ได้ทำใน `proxy.ts` ซึ่งต้องทำงานกับทุกคำขอ
ที่เข้าเว็บ หน้าที่มีอยู่จริงจึงไม่มีภาระเพิ่มแม้แต่นิดเดียว

ผลที่ตามมาคือ **เปลี่ยนเส้นทางของ URL ที่ยังมีหน้าอยู่ไม่ได้** ถ้าอยากพา `/about`
ไปที่อื่นต้องลบหรือเปลี่ยน slug ของหน้านั้นก่อน — ในทางปฏิบัติไม่เป็นปัญหา
เพราะทางเปลี่ยนเส้นทางมีไว้สำหรับลิงก์ที่ตายไปแล้ว

ตรวจด้วย `npm run check:redirects` (10 ข้อ)
...
| แท็บ | ฟิลด์ |
| --- | --- |
| ข้อมูลทั่วไป | `communityName` `communityShortName` `tagline` `logo` `favicon` `aboutSummary` |
...
> `favicon` คือไอคอนบนแท็บเบราว์เซอร์ ถ้าว่างใช้ `logo` ถ้าว่างทั้งคู่ใช้ใบโพธิ์ (`public/icon.svg`)
> ใช้รูปย่อจัตุรัส 320×320 ของคลังรูป จึงไม่ถูกบีบแม้รูปต้นฉบับไม่จัตุรัส
> ไอคอนประกาศผ่าน `generateMetadata` ใน layout — ห้ามวางไฟล์ `icon.*` ในโฟลเดอร์ `app`
> เพราะไฟล์แบบนั้นชนะค่าจาก CMS เสมอ
...
| **1ข. สีรายหน้า รายส่วน รายการ์ด** | พื้นหลัง สีเน้น และสีการ์ดของทุกหน้า ทุกส่วน แบนเนอร์ แถบเมนู และส่วนท้าย | ไม่ได้ | ผู้แก้ไขเนื้อหา |
...
| แถบเมนูด้านบน / ส่วนท้ายเว็บ | สีพื้น สีตัวอักษร และสีเน้นของสองส่วนนี้ (ดู "สีของทุกกล่อง" ด้านล่าง) |
...
- **สีของส่วนนี้** — กล่องพับที่มีชุดสีเดียวกับทุกกล่องของเว็บ (หัวข้อถัดไป)

#### สีของทุกกล่อง

ทุกหน้า ทุกส่วน และทุกกล่องที่มีการ์ดข้างใน เปลี่ยนสีได้ด้วย **ชุดฟิลด์สีชุดเดียวกัน**
(`colorFields` ใน `src/fields/index.ts`) และแปลงเป็น CSS ด้วยตัวแปลงตัวเดียว (`sectionSkin`)

| ช่อง | ทำอะไร |
| --- | --- |
| พื้นหลัง | ตามแบบเดิมของส่วนนี้ · พื้นอ่อนตัดกัน · พื้นเข้ม · กำหนดสีเอง |
| สีพื้นหลัง | เมื่อเลือก "กำหนดสีเอง" — พื้นสีอ่อนจะทำให้การ์ดและเส้นแบ่งในส่วนนั้นอมสีพื้นด้วย |
| สีตัวอักษรบนพื้นนี้ | อัตโนมัติ (ค่าเริ่มต้น) · สีอ่อน · สีเข้ม |
| สีเน้น | สีปุ่ม ป้าย และหัวข้อเล็กเฉพาะในกล่องนั้น (ไล่เฉด `--color-leaf-*` ใหม่บนตัวกล่อง) |
| สีการ์ดและกล่องข้างใน | สีพื้นของการ์ดทุกใบในส่วนนั้น — สีตัวอักษรบนการ์ดเลือกให้เองตามความสว่างของสี |

| ที่ตั้งค่า | มีผลกับ |
| --- | --- |
| แต่ละแถวใน "ลำดับและการแสดงส่วนต่าง ๆ" | ส่วนนั้นส่วนเดียว — **เปลี่ยนได้ทุกชนิดส่วน** ไม่จำกัดเฉพาะส่วนที่เป็นกริดแล้ว |
| บล็อกของหน้าที่สร้างเอง | บล็อกนั้นบล็อกเดียว |
| "สีของแถบหัวหน้าเพจ" / "สีของแบนเนอร์" | แบนเนอร์บนสุดของหน้านั้น (พื้นเข้มเป็นค่าเริ่มต้น) |
| "สีของทั้งหน้า" (`pageStyle`) | สีเน้น สีพื้น และสีการ์ดของทั้งหน้า — ส่วนที่ไม่ได้ตั้งสีเองใช้ค่านี้ หน้ารายละเอียดสินค้า/บทความยืมสีเน้นและสีพื้นโทนอ่อนจากหน้ารวม |
| ธีม → "แถบเมนูด้านบน" / "ส่วนท้ายเว็บ" | แถบเมนู เมนูมือถือ และส่วนท้าย ทุกหน้า (สีแถบสถานะของมือถือตามสีแถบเมนูด้วย) |

**ช่องสีในหลังบ้านเป็นจานสีให้คลิก** (`src/components/admin/ColorField.tsx`) มีสีอ่อน สีเข้ม
และสีเน้นที่เข้ากับเว็บให้เลือก พร้อมปุ่ม "สีอื่น…" (ตัวเลือกสีของเบราว์เซอร์) ช่องรหัสสี
และปุ่ม "ใช้ค่าเริ่มต้น" — ฐานข้อมูลยังเก็บเป็นรหัส `#rrggbb` เหมือนเดิม
ชุดสีแนะนำอยู่ที่ `src/lib/color.ts` ใช้ร่วมกับตัวเลือกสีบนหน้าเว็บ

**ตัวอักษรต้องอ่านออกเสมอ** — แต่ละคอมโพเนนต์มีคลาสสองชุด (บนพื้นอ่อน / บนพื้นเข้ม)
เลือกจากตาราง `INK` ใน `src/lib/tone.ts` ตามโทนที่ `sectionSkin` คำนวณ
ความสว่างเทียบด้วยค่าความต่างของสี (contrast) กับตัวอักษรทั้งสองแบบ ไม่ใช่ตัดที่ค่าเดียว

**สีการ์ดทำงานอย่างไร** — การ์ดทุกใบติดคลาส `box` ส่วนที่ตั้งสีการ์ดจะได้คลาส `has-card-color`
พร้อมตัวแปร `--card-bg` / `--card-line` แล้วกฎใน `globals.css` เปลี่ยนพื้นและเส้นขอบของการ์ด
ส่วนสีตัวอักษรบนการ์ด คอมโพเนนต์ถาม `skin.cardTone(ค่าเดิม)` ซึ่งคืนโทนตามสีการ์ด
(กล่องที่เดิมเป็นพื้นเข้ม เช่นกล่องชวนติดต่อและเรื่องเล่าเด่น จะเปลี่ยนเป็นแบบพื้นอ่อนเมื่อสีการ์ดเป็นสีอ่อน)

> **โค้ดที่เพิ่มใหม่** ต้องเลือกสีตัวอักษรจาก `INK[tone]` และติด `box` ที่การ์ด
> ถ้าเขียน `text-ink-800` ตายตัว ข้อความนั้นจะหายเมื่อผู้ดูแลเปลี่ยนพื้นเป็นสีเข้ม
...
### ชั้นที่ 4 — สร้างหน้าใหม่ทั้งหน้า (Collection `pages`)

หน้าประจำทั้งหกหน้ามีโครงตายตัวเพราะผูกกับดีไซน์เฉพาะของมัน ส่วนหน้าที่สร้างเอง
ประกอบจาก **บล็อก** ที่ลากสลับลำดับได้ จึงเพิ่มหน้าใหม่ได้โดยไม่ต้องแก้โค้ด

| บล็อก | ใช้ทำอะไร |
| --- | --- |
| เนื้อหาข้อความ | หัวข้อ ย่อหน้า และรายการ ผสมกันได้ในบล็อกเดียว |
| ภาพคู่ข้อความ | ภาพซ้ายหรือขวา พร้อมหัวเรื่อง เนื้อหา และปุ่ม |
| การ์ดหลายใบ | ไอคอน + หัวเรื่อง + เนื้อหา เลือกจำนวนคอลัมน์ได้ |
| ตัวเลขสำคัญ | ตัวเลขพร้อมคำอธิบาย |
| แกลเลอรีภาพ | ภาพหลายรูปเรียงเป็นกริด |
| ดึงเนื้อหาจากคลัง | หยิบสินค้า บทความ ฐานเรียนรู้ หรือจุดเช็กอินมาแสดง |
| กล่องชวนติดต่อ | กล่องปิดท้ายพร้อมปุ่ม LINE และโทร |

**ทุกบล็อกปรับสี ฟอนต์ ขนาดตัวอักษร การจัดวาง และความกว้างของตัวเองได้** ด้วยชุด
ตัวเลือกเดียวกับ section ของหน้าประจำ (`blockStyleFields` ใช้ชื่อฟิลด์ชุดเดียวกับ
`sectionsField` ตัวแปลงเป็น CSS จึงเป็นตัวเดียวกัน ไม่มีตรรกะสีสองชุด)

**ที่อยู่ของหน้า** มาจากช่อง "ตัวระบุใน URL" ในแถบด้านข้าง ใส่ `activity` จะได้ `/activity`
ระบบปฏิเสธชื่อที่ชนกับหน้าที่มีอยู่แล้ว (`/about`, `/shop`, `/admin` …) และชื่อที่มี `/`
เพราะเส้นทางที่เขียนไว้ในโค้ดถูกจับคู่ก่อนเสมอ — ถ้าปล่อยให้ตั้งชื่อซ้ำได้
หน้าที่สร้างใหม่จะไม่มีวันถูกเปิดเลย และผู้ดูแลจะงงว่าทำไมสร้างแล้วไม่ขึ้น

**เพิ่มเข้าเมนูได้** จากหน้า "เมนูนำทาง" เลือกประเภทลิงก์เป็น "หน้าที่สร้างเอง"
แล้วเลือกหน้าจากรายการ — ใช้ความสัมพันธ์ ไม่ใช่การพิมพ์ที่อยู่เอง
เปลี่ยนชื่อลิงก์ของหน้านั้นภายหลังเมนูจึงตามไปเอง ไม่กลายเป็นลิงก์เสีย

**อยู่ใน sitemap อัตโนมัติ** เว้นแต่ติ๊ก "ให้ Google เก็บหน้านี้" ออก ซึ่งจะใส่
`robots: noindex` ให้ด้วย ไม่ใช่แค่ตัดออกจาก sitemap เฉย ๆ

ตรวจด้วย `npm run check:pages` (28 ข้อ ครอบคลุมทุกบล็อก การจัดวาง sitemap
และการกันไม่ให้เส้นทางเดิมถูกแย่ง)

#### ทำไมไม่ทำตัวสร้างหน้าแบบลากวางอิสระ

ยืดหยุ่นกว่าก็จริง แต่ทำให้หน้าออกมาไม่เข้าชุดกับที่เหลือของเว็บได้ง่ายมาก
ชุดบล็อกที่จำกัดไว้แบบนี้ให้ผลลัพธ์ที่ยังดูเป็นเว็บเดียวกันเสมอ ซึ่งตรงกับแนวทาง
"ข้อมูลเชิงระบบ" ที่ตกลงกันไว้

### ยังต้องแก้โค้ดอยู่

เพิ่มบล็อกชนิดใหม่, เพิ่ม section ชนิดใหม่ในหน้าประจำ และเปลี่ยนโครงภายใน section
ที่มีอยู่ (เช่นย้ายภาพจากซ้ายไปขวาในหน้าแรก)

## 7. ข้อความบนปุ่มและป้ายกำกับ (Global `ui-labels`)

คำที่ใช้ซ้ำทั้งเว็บอย่าง "ผลิตโดย" "สเปกทางเทคนิค" "เวลาทำการ" "เงื่อนไขการจอง"
เคยเขียนตายตัวอยู่ในโค้ด ผู้ดูแลจึงเปลี่ยนเองไม่ได้เลยแม้จะเป็นแค่คำเดียว
ตอนนี้ย้ายมาอยู่ใน Global เดียว แบ่งเป็น 6 แท็บ รวม 48 คำ

| แท็บ | ตัวอย่างคำ |
| --- | --- |
| ทั่วไป | สอบถามราคา · ดูสินค้าทั้งหมด · เปิดนำทางด้วย Google Maps |
| สินค้า | ผลิตโดย · สเปกทางเทคนิค · ตัวกรอง · เรียงตาม · คำเตือนใช้ภายนอก |
| บทความ | เขียนโดย · ใช้เวลาอ่าน · แชร์บทความนี้ · เกี่ยวกับผู้เขียน |
| ท่องเที่ยว | ระยะเวลา · จำนวนผู้เข้าร่วม · ค่าบริการ · เงื่อนไขการจอง · ได้กลับบ้าน |
| ติดต่อและท้ายเว็บ | ที่ตั้ง · เวลาทำการ · ติดต่อชุมชน · สงวนลิขสิทธิ์ |
| เกี่ยวกับชุมชน | บทบาท · ที่มาข้อมูล · แหล่งอ้างอิง |

**ทุกคำมีค่าตั้งต้นอยู่ในโค้ด** (`DEFAULT_LABELS` ใน `src/lib/labels.ts`)
ช่องที่เว้นว่างหรือถูกลบข้อความออกจะตกกลับไปใช้คำตั้งต้น ไม่มีทางเห็นป้ายว่างเปล่า
และถ้าฐานข้อมูลถูกกู้คืนมาไม่ครบ หน้าเว็บก็ยังอ่านรู้เรื่อง

**แก้จากหน้าเว็บได้ด้วย** — เข้าโหมดแก้ไขแล้วคลิกที่คำนั้นบนหน้าจริงได้เลย

> **ข้อควรรู้เรื่องโครงไฟล์** — ค่าตั้งต้นและชนิดข้อมูลอยู่ที่ `src/lib/labels.ts`
> ส่วนตัวอ่านจากฐานข้อมูลอยู่ที่ `src/lib/cms/labels.ts` แยกกันโดยตั้งใจ
> เพราะไฟล์หลังเรียก Payload ซึ่งทำงานได้เฉพาะฝั่งเซิร์ฟเวอร์ ถ้าคอมโพเนนต์
> ฝั่งไคลเอนต์ import ไปด้วย Payload ทั้งก้อนจะถูกลากเข้าบันเดิลของเบราว์เซอร์แล้ว build พัง
> (เคยเกิดขึ้นจริงตอนทำ — `site-context.tsx` import ค่าตั้งต้นจากไฟล์ที่เรียก Payload)

---

## 8. ดูตัวอย่างก่อนเผยแพร่
...
| รูปที่มีกรอบประสีทอง | คลิกเพื่อเปลี่ยนรูป — เลือกจากคลังรูปหรืออัปโหลดใหม่ |
| ดินสอเล็กที่มุมโลโก้ | เปลี่ยนโลโก้ |
| ปุ่ม "สี" ที่ขอบบนซ้ายของทุกส่วน | เปิดแผงเลือกสีของส่วนนั้น (พื้นหลัง สีเน้น สีการ์ด) |
| "สี" ในเมนู "แก้ไขหน้านี้" | สีของทั้งหน้า แถบเมนูด้านบน และส่วนท้ายเว็บ |
| "รูปประจำเว็บ" ในเมนู "แก้ไขหน้านี้" | เปลี่ยนโลโก้ และไอคอนบนแท็บเบราว์เซอร์ (favicon) |
...
**สิ่งที่ยังต้องทำในหลังบ้าน** — เพิ่ม/ลบ/สลับลำดับรายการ (รวมถึงรูปในแกลเลอรี)
ภาพตอนแชร์ลิงก์ เปลี่ยนลิงก์ ลำดับ/คอลัมน์/ฟอนต์ของส่วน และตั้งค่าที่เป็นตัวเลือกอื่น ๆ
...
### เปลี่ยนรูปบนหน้าเว็บ

**ครอบคลุมอะไรบ้าง** — ทุกรูปที่ผูกกับช่องรูปเดี่ยวของเอกสาร: ภาพหลักหน้าแรก ภาพประกอบ
หน้าเกี่ยวกับชุมชน รูปสินค้า (ทีละรูปในแกลเลอรี) ภาพหน้าปกบทความ รูปในเนื้อหาบทความ
ภาพกิจกรรม ภาพสถานที่ รูปโปรไฟล์ปราชญ์ รูปในบล็อกของหน้าที่สร้างเอง รูปบนการ์ด
สินค้า/บทความทุกที่ โลโก้ และไอคอนเว็บ

```
คลิกที่รูป → หน้าต่างเลือกรูป ─┬─ แท็บ "เลือกจากคลังรูป"  ค้นหา → เลือก → "ใช้รูปนี้"
                              └─ แท็บ "อัปโหลดรูปใหม่"   ไฟล์ + คำบรรยาย + เจ้าของภาพ + สิทธิ์
                                                          → เข้าคลังรูปก่อน แล้วค่อยผูกเข้าช่อง
```

- ช่องที่ไม่บังคับ (ภาพหลักหน้าแรก ภาพประกอบประวัติ โลโก้ ไอคอนเว็บ รูปโปรไฟล์) มีปุ่ม **นำรูปออก**
  และเมื่อยังไม่มีรูป จะมีกล่อง "เพิ่มรูป" อยู่ตรงตำแหน่งนั้นในโหมดแก้ไข
- บันทึกเป็นฉบับร่างเหมือนการแก้ข้อความ แล้วกดปุ่มเขียวเผยแพร่
- หน้ารายละเอียดสินค้าและบทความมีปุ่ม "แก้ไขหน้านี้" ของตัวเองแล้ว จึงเผยแพร่จากหน้านั้นได้

**ทำไมอัปโหลดต้องเข้าคลังรูปก่อน** — ให้เหมือนหลังบ้าน: ทุกรูปบนเว็บต้องมีคำบรรยายภาพ
และเจ้าของภาพ ตรวจย้อนได้เสมอว่ารูปมาจากไหน (ดูหัวข้อ 4.2)

**ทำไมปุ่มวางทับรูป ไม่ห่อรูป** — การห่อรูปด้วยกล่องใหม่เปลี่ยนโครง layout ของทุกจุดบนเว็บ
ปุ่มจึงเป็น `absolute` วางในกล่อง `relative` เดิมของรูป ผู้เข้าชมทั่วไปได้ HTML เหมือนเดิมทุกตัวอักษร

#### ด่านตรวจของการเปลี่ยนรูปและอัปโหลด

ใช้ด่านเดียวกับการแก้ข้อความทั้งหมด (`src/lib/cms/inline-request.ts`) บวกด่านเฉพาะรูป

| ด่าน | กันอะไร |
| --- | --- |
| เทียบกับสคีมา: ต้องเป็น `upload` ที่ผูก `media` แบบรูปเดียว | เอาไปใส่ช่องข้อความ ช่องความสัมพันธ์ หรือช่องหลายรูป |
| บล็อกเลือกตามชนิดของแถวจริง | บล็อกต่างชนิดที่มีช่องชื่อเดียวกันแต่ตั้งค่าบังคับต่างกัน |
| รูปต้องมีอยู่จริงในคลังและไม่ใช่ `pending` | ผูกรูปที่ถูกลบไปแล้ว หรือรูปที่ยังไม่ได้ขออนุญาต |
| นำรูปออกได้เฉพาะช่องไม่บังคับ | ช่องรูปบังคับว่างจนหน้าเว็บพัง |
| อัปโหลด: ตรวจชนิดไฟล์จากเนื้อไฟล์ รับแค่ JPG/PNG/WebP/AVIF | SVG ที่ฝังสคริปต์ และไฟล์อื่นที่ปลอมนามสกุล |
| อัปโหลด: ไม่เกิน 8 MB, ตรวจ `Content-Length` ก่อนอ่าน | ไฟล์ใหญ่กินหน่วยความจำเซิร์ฟเวอร์ |
| อัปโหลด: บังคับคำบรรยายและเจ้าของภาพ สิทธิ์ต้องเป็น `own`/`licensed` | รูปที่ไม่รู้ที่มาขึ้นเว็บ |
| อัปโหลด: ชื่อไฟล์ตัดเหลือตัวอักษร/ตัวเลข นามสกุลตามชนิดจริง | ชื่อไฟล์แปลกปลอม |
| ลำดับแถวจริงในหลังบ้าน (`galleryRows`, `sourceIndex`) | เปลี่ยนรูปผิดแถว เมื่อมีแถวที่รูปถูกลบไปก่อนหน้า |

รันชุดตรวจได้ด้วย `npm run check:images` (44 ข้อ) — แก้ฉบับร่างชั่วคราวแล้วคืนค่าเดิม ไม่กดเผยแพร่

### เปลี่ยนสีบนหน้าเว็บ

กดปุ่ม **สี** ที่ขอบบนซ้ายของส่วน (หรือ "สี" ในเมนู "แก้ไขหน้านี้" สำหรับทั้งหน้า แถบเมนู และส่วนท้าย)
แผงเลือกสีเปิดที่มุมซ้ายล่างของจอ ไม่บังส่วนที่กำลังแก้ — เลือกแล้วหน้าดึงฉบับร่างใหม่ให้เห็นผลทันที

- ทุกการเลือกบันทึกเป็นฉบับร่าง แล้วไปรวมในปุ่มเขียว "เผยแพร่" เหมือนการแก้ข้อความ
- ตัวเลือกสีของเบราว์เซอร์รอให้หยุดลากก่อนค่อยบันทึก และบันทึกทีละคำขอ ค่าที่เลือกติดกันจึงไม่ทับกัน
- ปุ่ม "สี" ของส่วนต่าง ๆ ต้องมีแถวใน "ลำดับและการแสดงส่วนต่าง ๆ" อยู่จริง ถ้าหน้านั้นยังไม่เคยบันทึกลำดับ
  ให้กดบันทึกในหลังบ้านหนึ่งครั้งก่อน
- สีของแถบเมนูและส่วนท้ายอยู่ในธีม — ธีมแก้จากหน้าเว็บได้**เฉพาะช่องสี** (`STYLE_ONLY_GLOBALS`)
  ช่อง CSS เพิ่มเติมและช่องอื่นของธีมยังแก้ได้จากหลังบ้านเท่านั้น

| ด่าน | กันอะไร |
| --- | --- |
| ชื่อช่องต้องอยู่ในชุดฟิลด์สี และตรงสคีมาจริง (`findStyleField`) | ใช้ช่องทางนี้แก้ข้อความ ลำดับ หรือช่องอื่นของธีม |
| ช่องตัวเลือกรับเฉพาะค่าที่มีในรายการ | ค่าแปลกปลอมในช่องพื้นหลัง/สีตัวอักษร |
| ช่องสีรับเฉพาะ `#rrggbb` (หรือ `null` เพื่อกลับค่าเริ่มต้น) | แทรก CSS ผ่านค่าสี — ค่าสีถูกเขียนลงสไตล์ของหน้าตรง ๆ |
| ทุกช่องต้องอยู่ใต้กิ่งบนสุดเดียวกัน และไม่สร้างแถวใหม่ | เขียนทับหลายส่วนของเอกสารในคำขอเดียว |

รันชุดตรวจได้ด้วย `npm run check:styles` (35 ข้อ) — สคริปต์กดเผยแพร่ธีมจริงหนึ่งครั้ง แล้วคืนค่าเดิม
...
| รายการเอกสารที่อนุญาต | ผู้ใช้ถูกกันออกตั้งแต่ต้น ธีมแก้ได้เฉพาะช่องสีผ่านคำขอเปลี่ยนสี |
...
| `src/components/inline-image.tsx` | ปุ่มเปลี่ยนรูป กล่องเพิ่มรูป และหน้าต่างเลือก/อัปโหลดรูป |
| `<EdImage>` ใน `editable.tsx` | ตัดสินฝั่งเซิร์ฟเวอร์ว่าจะวางปุ่มเปลี่ยนรูปหรือไม่ |
| `src/lib/cms/inline-request.ts` | ด่านตรวจร่วม: โดเมน การล็อกอิน บทบาท |
| `/api/inline-edit` | บันทึกข้อความ เปลี่ยนรูป เปลี่ยนสี และเผยแพร่ |
| `src/components/style-editor.tsx` | ปุ่ม "สี" และแผงเลือกสีบนหน้าเว็บ |
| `<EdStyle>` ใน `editable.tsx` | ตัดสินฝั่งเซิร์ฟเวอร์ว่าจะวางปุ่ม "สี" หรือไม่ |
| `src/components/page-style.tsx` | สีของทั้งหน้า (ฉีดเป็น `<style>` ของหน้านั้น) |
| `src/lib/color.ts` · `src/lib/tone.ts` | คำนวณสี ชุดสีแนะนำ และคลาสตัวอักษรตามโทนของพื้น |
| `/api/inline-media` | รายการคลังรูป และอัปโหลดรูปใหม่ |
...
### หน้าจอหลังบ้าน: โหมดสว่าง/มืด

แถบด้านบนของหลังบ้านมีปุ่ม **ตามเครื่อง · สว่าง · มืด** (`src/components/admin/ThemeToggle.tsx`)
ค่าที่เลือกเก็บในคุกกี้ `payload-theme` ของเบราว์เซอร์นั้น — แต่ละคนเลือกของตัวเองได้
ไม่กระทบคนอื่น และไม่กระทบหน้าเว็บสาธารณะ "ตามเครื่อง" ลบคุกกี้แล้วใช้โหมดของระบบปฏิบัติการ

**เมนูด้านข้างเปิดค้างบนโน้ตบุ๊ก** — Payload ปิดเมนูเองทุกครั้งที่โหลดหน้าเมื่อจอกว้างไม่เกิน 1440px
ผู้ดูแลจึงเปิดมาเจอแต่แดชบอร์ด `src/components/admin/NavKeeper.tsx` เปิดกลับให้ในช่วงจอ 1025–1440px
และจำการกดปิด/เปิดของแต่ละคนไว้ในค่าเดียวกับที่ Payload ใช้บนจอกว้าง (จอ ≤1024px ยังเป็นเมนูลอยทับตามเดิม)
บนสุดของเมนูมีลิงก์ **แดชบอร์ด** (`DashboardLink.tsx`) เพราะเมนูของ Payload ไม่มีทางกลับหน้าแรกหลังบ้าน
...
## 11. ดูตัวอย่างและอัปเดตหน้าเว็บ
...
## 14. การขึ้นเซิร์ฟเวอร์และโดเมนทดสอบ

ขั้นตอนละเอียดอยู่ใน **[docs/deploy-plesk.md](./deploy-plesk.md)** — สรุปสาระสำคัญที่มีผลกับการออกแบบ CMS
...
**ไฟล์ฐานข้อมูลและรูปที่อัปโหลดต้องอยู่นอกโฟลเดอร์แอป** — deploy คือการเขียนทับ
...
**build ที่เครื่องตัวเอง ไม่ build บนเซิร์ฟเวอร์** — โฮสต์แบบแชร์จำกัดหน่วยความจำต่อโพรเซส
`next build` มีโอกาสถูกฆ่ากลางทาง ใช้ `npm run deploy` ซึ่ง build แล้ว rsync ขึ้นไปให้

**หน้าสินค้าและบทความถูกสร้างตอน build โดยอ่านจากฐานข้อมูล** — สคริปต์ deploy จึงดาวน์โหลด
ไฟล์ฐานข้อมูลของเซิร์ฟเวอร์ลงมาอ่านผ่าน FTP (อ่านอย่างเดียว ไม่เขียนกลับ) แล้วลบสำเนาทิ้งเมื่อจบ

**บิลด์บน Mac ต้องเติมไฟล์ native ของ Linux** — SQLite (`libsql`) และตัวย่อรูป (`sharp`)
มีไฟล์เฉพาะระบบปฏิบัติการ บิลด์บน Mac ได้ของ macOS มา สคริปต์ deploy จึงดาวน์โหลดของ Linux
ทั้ง x64 และ arm64 มาแทน และแปลง symlink ในบิลด์เป็นไฟล์จริงเพราะ FTP สร้าง symlink ไม่ได้
(ทดสอบแล้วด้วย Docker `node:24-bookworm-slim` ทั้งสองสถาปัตยกรรม)
...
ทั้งเว็บอยู่ในโฟลเดอร์ `bantonpoo-data/` เดียว (ไฟล์ฐานข้อมูล + รูปที่อัปโหลด)
ซึ่งอยู่ในบ้านของ subscription **Plesk Backup Manager จึงสำรองให้ครบโดยไม่ต้องตั้งอะไรเพิ่ม**

ดึงสำเนาลงมาเก็บเองได้ทุกเมื่อ
...
รันได้ทุกตัวขณะ `npm run dev` ทำงานอยู่ — ทุกตัวสร้างข้อมูลทดสอบเองแล้วลบทิ้งเมื่อจบ
รวมถึงกรณีที่สคริปต์พังกลางทาง

| คำสั่ง | ตรวจอะไร | จำนวนข้อ |
| --- | --- | --- |
| `npm run check:public` | หน้าสาธารณะไม่มีลิงก์หลังบ้านหรือที่อยู่ฟิลด์หลุด | 8 หน้า |
| `npm run check:inline` | การแก้ข้อความบนหน้าเว็บ สิทธิ์ ฉบับร่าง เผยแพร่ และด่านปฏิเสธ | 20 |
| `npm run check:images` | การเปลี่ยนรูป อัปโหลด คลังรูป ไอคอนเว็บ และด่านปฏิเสธ | 44 |
| `npm run check:styles` | การเปลี่ยนสีส่วน แบนเนอร์ ทั้งหน้า แถบเมนู ฉบับร่าง เผยแพร่ และด่านปฏิเสธ | 35 |
| `npm run check:redirects` | ทางเปลี่ยนเส้นทาง รหัสตอบกลับ และการกันวนไม่รู้จบ | 10 |
| `npm run check:pages` | หน้าที่สร้างเอง ทุกบล็อก การจัดวาง sitemap การแก้ในหน้าเว็บ และการกันเส้นทางชนกัน | 28 |

พร้อมกับ `npm run lint`, `npm run typecheck` และ `npm run build` ตามปกติ

> `check:inline` `check:images` `check:styles` และ `check:pages` แก้เนื้อหาจริงชั่วคราว — `check:inline`
> กดเผยแพร่หน้าแรกหนึ่งครั้ง และ `check:styles` กดเผยแพร่ธีมหนึ่งครั้ง ถ้ามีฉบับร่างของเอกสารนั้นค้างอยู่
> ฉบับร่างนั้นจะถูกเผยแพร่ไปด้วย
>
> ตอนคืนค่าฉบับเผยแพร่ต้องใส่ `_status: "published"` เอง — Payload ยึดสถานะจากฉบับล่าสุด
> (ซึ่งเป็นฉบับร่างที่การตรวจสร้างไว้) ถ้าไม่ระบุ จะได้ฉบับร่างค้างที่หลังบ้านขึ้นว่า "มีการแก้ที่ยังไม่เผยแพร่"
