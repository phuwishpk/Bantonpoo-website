...
**ตัวอักษรและการจัดวางรายส่วน** อยู่ในกล่องพับ "ตัวอักษรและการจัดวางของส่วนนี้"
ของแต่ละแถว และในแบนเนอร์หัวหน้าเพจของทุกหน้า

| ตัวเลือก | ค่าที่เลือกได้ | ทำอะไรกับหน้าเว็บ |
| --- | --- | --- |
| ฟอนต์ | ตามธีม · Plex+Noto · Sarabun+Trirong · Prompt | เขียนทับ `--font-sans` / `--font-serif` เฉพาะในส่วนนั้น |
| ขนาดตัวอักษร | 0.9 · 0.95 · ปกติ · 1.1 · 1.2 เท่า | คูณ `--text-*` ทุกขนาดพร้อมกัน สัดส่วนหัวเรื่องกับเนื้อหาจึงไม่เพี้ยน |
| การจัดวางข้อความ | ตามค่าเริ่มต้น · ซ้าย · กึ่งกลาง · ขวา | ใส่คลาส `text-left/center/right` ที่ตัว `<section>` |
| ความกว้างของเนื้อหา | ตามค่าเริ่มต้น · แคบ (48rem) · ปานกลาง (64rem) · เต็มพื้นที่ | ตั้ง `--section-measure` ให้ `Container` อ่านไปใช้ |
| ความโปร่ง | ตามธีม · กระชับ · มาตรฐาน · โปร่ง | เขียนทับ `--spacing` ซึ่ง Tailwind ใช้คำนวณระยะห่างทุกคลาส |

**ทุกช่องเป็นตัวเลือกจำกัด ไม่ใช่ช่องพิมพ์อิสระ** โดยตั้งใจ — ถ้าเปิดให้พิมพ์ขนาดฟอนต์
หรือชื่อฟอนต์ได้เอง แต่ละส่วนของเว็บจะหลุดจากกันเร็วมาก และฟอนต์ที่ไม่ได้โหลดมาพร้อมเว็บ
จะทำให้หน้าช้าลงด้วย

> **ขนาดตัวอักษรที่เขียนตายตัวจะไม่ขยับ** — คลาสแบบ `text-[1.1rem]` ไม่ได้อ้างตัวแปร
> จึงไม่ถูกคูณตามค่าที่เลือก ดีไซน์นี้จึงย้ายขนาดที่ใช้บ่อยไปเป็นโทเคนใน `globals.css`
> แล้ว (`text-2xs`, `text-md`, `text-display-sm/md/lg`) โค้ดที่เพิ่มใหม่ควรใช้โทเคนเหล่านี้
...
| ข้อความที่มีกรอบประ | คลิกแล้วพิมพ์ทับได้เลย ไม่ต้องเข้าหลังบ้าน |
| ปุ่มเขียว "เผยแพร่ N รายการที่แก้ไว้" | เผยแพร่ทุกเอกสารที่เพิ่งแก้จากหน้าเว็บในครั้งเดียว |

กลับมาที่แท็บเว็บเมื่อไหร่ หน้าจะดึงข้อมูลใหม่ให้เอง จึงเห็นผลการแก้ทันทีโดยไม่ต้องกดรีเฟรช

### แก้ข้อความบนหน้าเว็บโดยตรง

**ครอบคลุมอะไรบ้าง** — ข้อความสั้น ย่อหน้ายาว และรายการ (เช่น วิธีใช้ ข้อควรระวัง
เงื่อนไขการจอง แหล่งอ้างอิง) รวมถึงเมนูบน ปุ่มมุมขวาบน เมนูบนมือถือ และท้ายเว็บ
ทั้งในหน้าหลักทั้ง 6 หน้า หน้ารายละเอียดสินค้า และหน้าบทความ

| ปุ่ม | ผล |
| --- | --- |
| คลิกที่ข้อความ | เข้าโหมดพิมพ์ |
| `Enter` | บันทึก (ช่องย่อหน้ายาวใช้ `Ctrl+Enter` ส่วน `Enter` เป็นการขึ้นบรรทัดใหม่) |
| `Esc` | ยกเลิก คืนข้อความเดิม |
| คลิกที่อื่น | บันทึกให้อัตโนมัติ |

กรอบจะเปลี่ยนเป็นสีเขียวเมื่อบันทึกสำเร็จ และเป็นสีแดงพร้อมข้อความบอกเหตุเมื่อไม่สำเร็จ

**ทุกการแก้ลงเป็นฉบับร่าง** ผู้เข้าชมทั่วไปยังไม่เห็นจนกว่าจะกดปุ่มเขียวเผยแพร่
(เอกสารที่ไม่ได้เปิดระบบฉบับร่างไว้ เช่น จุดเช็กอินและฐานเรียนรู้ จะขึ้นเว็บทันที)

**สิ่งที่ยังต้องทำในหลังบ้าน** — เพิ่ม/ลบ/สลับลำดับรายการ เปลี่ยนรูป เปลี่ยนลิงก์
ปรับสีและ layout ของส่วน และตั้งค่าที่เป็นตัวเลือกทั้งหมด

**ทำไมไม่บันทึกระหว่างพิมพ์** — ภาษาไทยพิมพ์ทีละอักขระแล้วประกอบเป็นคำ
ถ้ายิงบันทึกทุกครั้งที่พิมพ์ จะได้คำที่ยังไม่ครบเก็บลงฐานข้อมูลเต็มไปหมด
และผู้ใช้กด "เลิกทำ" ไม่ทัน จึงบันทึกตอนคลิกออกหรือกด Enter เท่านั้น

#### ด่านตรวจของ `/api/inline-edit`

| ด่าน | กันอะไร |
| --- | --- |
| `payload.auth()` + ตรวจบทบาท | ผู้ไม่ได้ล็อกอิน และบัญชี `viewer` ที่ดูได้อย่างเดียว |
| ตรวจ `Origin` กับ `Host` | เว็บอื่นยิงคำขอแทนผู้ใช้ที่ล็อกอินค้างไว้ |
| รายการเอกสารที่อนุญาต | ธีม (ช่อง CSS) และผู้ใช้ ถูกกันออกตั้งแต่ต้น |
| รูปแบบเส้นทาง | ชื่อฟิลด์ต้องเป็นตัวระบุธรรมดาหรือเลขลำดับ ลึกไม่เกิน 8 ชั้น |
| เทียบกับสคีมาของ Payload | ชื่อฟิลด์ที่พิมพ์ผิด และช่องที่ไม่ใช่ `text`/`textarea` |
| ส่งกลับเฉพาะกิ่งบนสุด | ไม่ทับฟิลด์อื่นที่คนอื่นอาจกำลังแก้อยู่พร้อมกัน |
| ไม่สร้างแถวในอาร์เรย์ | เส้นทางที่ชี้ไปยังแถวที่ยังไม่มี ถูกปฏิเสธแทนที่จะสร้างแถวไม่ครบฟิลด์ |
| จำกัดความยาว 6,000 ตัวอักษร | การยัดข้อมูลก้อนใหญ่ผ่านช่องทางนี้ |

รันชุดตรวจทั้งหมดได้ด้วย `npm run check:inline` (ต้องมี `npm run dev` ทำงานอยู่)
สคริปต์สร้างผู้ใช้ทดสอบชั่วคราวเอง แล้วลบทิ้งเมื่อจบ

> **หมายเหตุ** — สคริปต์นี้กดเผยแพร่หน้าแรกจริงหนึ่งครั้งเพื่อตรวจปลายทาง
...
| `src/lib/cms/inline.ts` | รูปแบบที่อยู่ของฟิลด์ + รายการเอกสารที่แก้จากหน้าเว็บได้ |
| `src/lib/cms/inline-schema.ts` | เดินผังฟิลด์ของ Payload เพื่อยืนยันว่าเส้นทางชี้ไปยังช่องข้อความจริง |
| `src/components/editable.tsx` | `<Ed>` — ตัดสินฝั่งเซิร์ฟเวอร์ว่าจะสร้างช่องแก้ไขหรือส่งข้อความล้วน |
| `src/components/inline-editable.tsx` | ช่องพิมพ์บนหน้าเว็บ พร้อมบันทึก ยกเลิก และสถานะ |
| `/api/inline-edit` | บันทึกฉบับร่างและเผยแพร่ |
