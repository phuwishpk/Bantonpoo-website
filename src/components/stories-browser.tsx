  ...
  return (
    <div className="flex flex-col gap-8">
            ...
            return (
              <button
                ...
                className={`shrink-0 rounded-full px-4 py-2.5 text-sm font-medium transition duration-200 ease-craft ${
                  active
                    ? "bg-steel-800 text-rice-100"
                    : "border border-rice-300 bg-rice-50 text-steel-700 hover:border-steel-400"
                }`}
              >
                {tab.label}
              </button>
      ...
      <p className="text-sm text-forged-500" aria-live="polite">
        พบ <span className="font-semibold text-steel-800">{visible.length}</span> บทความ
      </p>
      ...
      {visible.length > 0 ? (
        ...
        <div className="rounded-card border border-dashed border-rice-400 bg-rice-50 px-6 py-16 text-center">
          <p className="font-serif text-lg text-steel-800">ไม่พบบทความที่ตรงกับคำค้น</p>
          <p className="mt-2 text-sm text-forged-500">ลองใช้คำที่สั้นลง หรือเลือกดูจากหมวดหมู่ทั้งหมด</p>
          <button
            type="button"
            onClick={() => {
              setCategory(null);
